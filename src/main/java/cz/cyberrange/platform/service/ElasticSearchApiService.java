...
public class ElasticSearchApiService {
  ...
  public List<OverallPhaseStatistics> getOverAllPhaseStatistics(
      long trainingRunId, List<Long> phaseIds, String accessToken, Long userId) {
    try {
          ...
          .uri(
              uriBuilder ->
                  uriBuilder
                      ...
                      .queryParam(
                          "phaseIds", StringUtils.collectionToDelimitedString(phaseIds, ","))
      ...
      throw new MicroserviceApiException(
          "Could not retrieve wrong answers statistics from elastic for training run "
              + trainingRunId
              + ".",
          ex);
    }
