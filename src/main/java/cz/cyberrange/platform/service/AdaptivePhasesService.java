...
public class AdaptivePhasesService {
   ...
   */
  public List<List<SuitableTaskResponseDto>> computeSuitableTask(
      List<OverallInstancePerformance> overallInstancePerformances) {
    List<List<SuitableTaskResponseDto>> suitableTasksForTrainees = new ArrayList<>();
    overallInstancePerformances.forEach(
        trainee -> {
          List<SuitableTaskResponseDto> suitableTasksResponse = new ArrayList<>();
          trainee
              .getSmartAssistantInput()
              .forEach(
                  smartAssistantInput -> {
                    double participantsPerformance =
                        computeParticipantsPerformance(
                            smartAssistantInput, trainee.getPhasesSmartAssistantInput());
                    suitableTasksResponse.add(
                        findSuitableTask(participantsPerformance, smartAssistantInput));
                  });
   ...
   */
  public SuitableTaskResponseDto computeSuitableTask(
      AdaptiveSmartAssistantInput smartAssistantInput, String accessToken, Long userId) {
    double participantsPerformance =
        computeParticipantsPerformance(
            smartAssistantInput, accessToken, userId); // must be in interval <0,1>
    return findSuitableTask(participantsPerformance, smartAssistantInput);
  }

  private double computeParticipantsPerformance(
      AdaptiveSmartAssistantInput smartAssistantInput, String accessToken, Long userId) {
    Map<Long, OverallPhaseStatistics> overAllPhaseStatistics =
        elasticSearchApiService
            .getOverAllPhaseStatistics(
  ...
  private double computeParticipantsPerformance(
      AdaptiveSmartAssistantInput smartAssistantInput,
      Map<Long, OverallPhaseStatistics> overallPhaseStatistics) {
    return evaluateParticipantPerformance(smartAssistantInput, overallPhaseStatistics);
  }

  /**
   * This method is equal to the third equation in the paper:
   * https://www.muni.cz/en/research/publications/1783806
   */
  private SuitableTaskResponseDto findSuitableTask(
      double participantsPerformance, AdaptiveSmartAssistantInput smartAssistantInput) {
    ...
    if (participantsPerformance == ZERO) {
      ...
      // the (int) cast is equal to the trunc function in the third equation in the above-mentioned
      // paper
      int suitableTask =
          ((int) (smartAssistantInput.getPhaseXTasks() * (1 - participantsPerformance))) + 1;
      suitableTaskResponseDto.setSuitableTask(suitableTask);
    }
    return suitableTaskResponseDto;
  }

  private double evaluateParticipantPerformance(
      AdaptiveSmartAssistantInput smartAssistantInput,
      Map<Long, OverallPhaseStatistics> overAllPhaseStatistics) {
    ...
    long currentPhaseId =
        smartAssistantInput
            .getDecisionMatrix()
            .get(smartAssistantInput.getDecisionMatrix().size() - 1)
            .getRelatedPhaseInfo()
            .getId();
    for (DecisionMatrixRowDTO decisionMatrixRow : smartAssistantInput.getDecisionMatrix()) {
      ...
      participantWeightedPerformance +=
          decisionMatrixRow.getQuestionnaireAnswered()
              * convertBooleanToBinaryDouble(
                  relatedPhaseInfo.isCorrectlyAnsweredRelatedQuestions());
      ...
      OverallPhaseStatistics relatedPhaseStatistics =
          Optional.ofNullable(overAllPhaseStatistics.get(relatedPhaseInfo.getId()))
              .orElseThrow(
                  () ->
                      new EntityNotFoundException(
                          new EntityErrorDetail(
                              OverallPhaseStatistics.class,
                              "id",
                              Long.class,
                              relatedPhaseInfo.getId(),
                              "Statistics for phase not found")));
      if (!relatedPhaseStatistics.getSolutionDisplayed()) {
        participantWeightedPerformance +=
            evaluateSolutionDisplayed(decisionMatrixRow, relatedPhaseStatistics);
        participantWeightedPerformance +=
            evaluateKeywordUsed(decisionMatrixRow, relatedPhaseStatistics);
        participantWeightedPerformance +=
            evaluateCompletedInTime(
                decisionMatrixRow,
                relatedPhaseStatistics,
                relatedPhaseInfo.getEstimatedPhaseTime());
        participantWeightedPerformance +=
            evaluateWrongAnswers(decisionMatrixRow, relatedPhaseStatistics);
      }
      sumOfAllWeights +=
          decisionMatrixRow.getCompletedInTime()
              + decisionMatrixRow.getSolutionDisplayed()
              + decisionMatrixRow.getKeywordUsed()
              + decisionMatrixRow.getWrongAnswers();
    }
    if (sumOfAllWeights == 0) {
      log.error(
          "No weights found for adaptive smart assistant input {}. The easiest task will be picked",
          smartAssistantInput);
      return 0.0;
    }
  ...
  private double evaluateCompletedInTime(
      DecisionMatrixRowDTO decisionMatrixRow,
    ...
    return decisionMatrixRow.getCompletedInTime()
        * convertBooleanToBinaryDouble(phaseStatistics.getPhaseTime() < estimatedTimeInMillis);
  }

  private double evaluateKeywordUsed(
      DecisionMatrixRowDTO decisionMatrixRow, OverallPhaseStatistics phaseStatistics) {
    return decisionMatrixRow.getKeywordUsed()
        * convertBooleanToBinaryDouble(
            phaseStatistics.getNumberOfCommands() < decisionMatrixRow.getAllowedCommands());
  }

  private double evaluateSolutionDisplayed(
      DecisionMatrixRowDTO decisionMatrixRow, OverallPhaseStatistics phaseStatistics) {
    return decisionMatrixRow.getSolutionDisplayed()
        * convertBooleanToBinaryDoubleNegated(phaseStatistics.getSolutionDisplayed());
  }

  private double evaluateWrongAnswers(
      DecisionMatrixRowDTO decisionMatrixRow, OverallPhaseStatistics phaseStatistics) {
    return decisionMatrixRow.getWrongAnswers()
        * convertBooleanToBinaryDouble(
            phaseStatistics.getWrongAnswers().size() < decisionMatrixRow.getAllowedWrongAnswers());
  }
