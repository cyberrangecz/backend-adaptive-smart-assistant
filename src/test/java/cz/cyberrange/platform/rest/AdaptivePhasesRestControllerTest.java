...
class AdaptivePhasesRestControllerTest {
  ...
  private MockMvc mvc;
  @Autowired private AdaptivePhasesRestController adaptivePhasesRestController;
  ...
  public void init() {
    this.mvc =
        MockMvcBuilders.standaloneSetup(adaptivePhasesRestController)
  ...
  void findSuitableTaskFirstTrainingPhaseInTrainingDefinition() throws Exception {
    given(
            elasticSearchApiService.getOverAllPhaseStatistics(
                anyLong(), anyList(), anyString(), anyLong()))
        .willReturn(Collections.emptyList());
    MockHttpServletResponse response =
        mvc.perform(
                post("/adaptive-phases")
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                    .content(
                        ObjectConverter.convertObjectToJson(
                            testDataFactory.getAdaptiveSmartAssistantInput1())))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    SuitableTaskResponseDto suitableTaskResponseDto =
        ObjectConverter.convertJsonToObject(
            response.getContentAsString(), SuitableTaskResponseDto.class);
    assertThat(
        suitableTaskResponseDto, is(equalTo(testDataFactory.getSuitableTaskResponseThree())));
  }
  ...
  void findSuitableTaskSecondTrainingPhaseInTrainingDefinition() throws Exception {
    given(
            elasticSearchApiService.getOverAllPhaseStatistics(
                anyLong(), anyList(), anyString(), anyLong()))
        .willReturn(testDataFactory.getOverallPhaseStatisticsAllCorrect());
    MockHttpServletResponse response =
        mvc.perform(
                post("/adaptive-phases")
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                    .content(
                        ObjectConverter.convertObjectToJson(
                            testDataFactory.getAdaptiveSmartAssistantInput2())))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    SuitableTaskResponseDto suitableTaskResponseDto =
  ...
  void findSuitableTaskWorstPossiblePerformance() throws Exception {
    given(
            elasticSearchApiService.getOverAllPhaseStatistics(
                anyLong(), anyList(), anyString(), anyLong()))
        .willReturn(testDataFactory.getOverallPhaseStatisticsAllWrong());
    MockHttpServletResponse response =
        mvc.perform(
                post("/adaptive-phases")
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                    .content(
                        ObjectConverter.convertObjectToJson(
                            testDataFactory.getAdaptiveSmartAssistantInput3())))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    SuitableTaskResponseDto suitableTaskResponseDto =
  ...
  void findSuitableTaskDifferentWeightsInMatrix() throws Exception {
    given(
            elasticSearchApiService.getOverAllPhaseStatistics(
                anyLong(), anyList(), anyString(), anyLong()))
        .willReturn(testDataFactory.getOverallPhaseStatisticsCombinedPerformance());
    MockHttpServletResponse response =
        mvc.perform(
                post("/adaptive-phases")
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                    .content(
                        ObjectConverter.convertObjectToJson(
                            testDataFactory.getAdaptiveSmartAssistantInput4())))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    SuitableTaskResponseDto suitableTaskResponseDto =
        ...
        mvc.perform(
                post("/adaptive-phases")
                    ...
                    .content(
                        ObjectConverter.convertObjectToJson(
                            testDataFactory
                                .getAdaptiveSmartAssistantInputZeroWeightsInDecisionMatrix())))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    SuitableTaskResponseDto suitableTaskResponseDto =
  ...
  void findSuitableTaskWrongPhaseIds() throws Exception {
    AdaptiveSmartAssistantInput adaptiveSmartAssistantInput =
        testDataFactory.getAdaptiveSmartAssistantInput2();
    adaptiveSmartAssistantInput.getDecisionMatrix().get(0).setCompletedInTime(1L);
    given(
            elasticSearchApiService.getOverAllPhaseStatistics(
                anyLong(), anyList(), anyString(), anyLong()))
        .willReturn(testDataFactory.getOverallPhaseStatisticsWrongPhaseId());
    MockHttpServletResponse response =
        mvc.perform(
                post("/adaptive-phases")
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
    ...
    ApiEntityError apiEntityError =
        ObjectConverter.convertJsonToObject(response.getContentAsString(), ApiEntityError.class);
